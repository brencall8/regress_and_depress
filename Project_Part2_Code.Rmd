...
```{r}
fit.one <- lm(SoldPrice ~ Sqft + YearBuilt + Bath  + Parking + HomeTypeNew, data  = traindata)
...
```{r}
boxCox(lm(SoldPrice~Sqft+YearBuilt+HomeTypeNew, data = traindata))
```
...
```{r}
fit.two <- lm(I(sqrt(SoldPrice)) ~ Sqft + YearBuilt + Bath  + Parking + HomeTypeNew, data  = traindata)

plot(fit.two)
```

```{r}
realestateNew <- traindata %>%
  select(-c(HomeType, LotSize))
