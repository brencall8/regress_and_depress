...
pairs(SoldPrice ~ Sqft + Bed + Bath + YearBuilt + LotSize + Parking, data = realestate, 
      col = c('aquamarine', 'cornflowerblue','royalblue', 'cadetblue', 'cyan', 
              'darkblue', 'deepskyblue', 'darkseagreen', 'darkseagreen1')[factor(realestate_clean$HomeTypeNew)], 
      pch = 19)
```
...
bonferonni <- (1 - 0.05 / 5)
confint(model, level = bonferonni)
```
...
```{r}
#Model for validation statistics
...
full_model <- lm(I(sqrt(SoldPrice)) ~ Sqft + LotSize + HomeTypeNew, data = realestate_clean)
summary(full_model)
```
...
```{r}
#coefficients table
data.frame("FullModel" = full_model$coefficients, "TrainingModel" = model$coefficients)
```
